...
class AuthService:
    def register_user(self, db: Session, user_data: UserCreate) -> UserResponse:
        if UserRepository.get_by_email(db, user_data.email):
            raise ConflictException(
                "Email already registered.", ErrorCode.USER_ALREADY_EXISTS)
        ...
        return UserResponse.model_validate(UserRepository.create(db, user))
    ...
    def login_user(self, db: Session, login_data: LoginRequest) -> Token:
        ...
        return self._create_tokens(db, user)

    def refresh_access_token(self, db: Session, request: RefreshTokenRequest) -> Token:
        stored_token = RefreshTokenRepository.get_token(
            db, request.refresh_token)
        if not stored_token:
            raise UnauthorizedException(
                "Invalid refresh token.", ErrorCode.INVALID_TOKEN)

        payload = decode_token(request.refresh_token)
        if payload.get("type") != "refresh":
            raise UnauthorizedException(
                "Invalid token type.", ErrorCode.INVALID_TOKEN)
    ...
    def logout_user(self, db: Session, refresh_token: str):
        stored_token = RefreshTokenRepository.get_token(db, refresh_token)
        if stored_token:
            RefreshTokenRepository.delete(db, stored_token)
        return {"message": "Logout successful."}

    def _create_tokens(self, db: Session, user: User) -> Token:
        access_token = create_access_token(
            {"sub": str(user.id), "email": user.email, "role": user.role})
